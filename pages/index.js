    ...
    <div style={{ maxWidth: '1000px', margin: '0 auto', padding: '20px', fontFamily: 'sans-serif' }}>
      ...
      <div style={{ display: 'flex', gap: '10px', margin: '20px 0' }}>
        <input
          ...
          style={{ flex: 1, padding: '12px 14px', border: '1px solid #ddd', borderRadius: '10px', outline: 'none' }}
        />
        <button onClick={search} style={{ padding: '12px 24px', background: '#0a0a23', color: 'white', borderRadius: '10px', fontWeight: 'bold', border: 'none', cursor: 'pointer' }}>
          Search
        </button>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(170px, 1fr))', gap: '15px' }}>
        {display.map((p, i) => (
          <div key={i} style={{ border: '1px solid #e5e5e5', borderRadius: '16px', padding: '12px', background: 'white' }}>
            ...
            <a href={p.productLink || p.product_link} target="_blank" rel="noreferrer" style={{ display: 'block', textAlign: 'center', background: '#f5f5f5', padding: '8px', borderRadius: '8px', textDecoration: 'none', color: '#0a0a23', fontWeight: 'bold', fontSize: '13px' }}>
              View More
            </a>
          </div>
        ))}
